...
class CassetteCreate(BaseModel):
    ...
    year: int
    status_id: int  # теперь ссылка на таблицу statuses
...
def create_cassette(cassette: CassetteCreate):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # проверка статуса
        cursor.execute(
            "SELECT 1 FROM statuses WHERE id_status = ?",
            (cassette.status_id,)
        )
        ...
        # проверка шкафа
        cursor.execute(
            "SELECT 1 FROM cabinets WHERE id_cabinet = ?",
            (cassette.cabinet_id,)
        )
        if not cursor.fetchone():
        ...
        # проверка жанра
        cursor.execute(
            "SELECT 1 FROM genres WHERE id_genre = ?",
            (cassette.genre_id,)
        )
        if not cursor.fetchone():
...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        #Заполняем справочник статусов (если пусто)
